...
$admin_id = mysqli_real_escape_string($conn, $_SESSION['id']);
  if($_SERVER['REQUEST_METHOD']==="POST"){
    ...
    $sql="Select * from student where email='$email' OR (firstname='$fname' and lastname='$lname')";
    $result=mysqli_query($conn, $sql);

    if(mysqli_num_rows($result)==0){
    $sql= "INSERT INTO student() VALUES (null,'$fname', '$lname', '$email','$grp_id', '$admin_id')";
...
$sql="Select * from cl_group WHERE admin_id = '$admin_id' order by id asc";
$result=mysqli_query($conn, $sql);
...
?>
  ...
  <div class="container-fluid">
    ...
    <div class="alert alert-primary" role="alert">
      Click on the number of Student to see the list of Student in a group. One student can only be in one group and not many
    </div>
      <div class="col-4">
        Group Number
      </div>
    ...
    <?php
        ...
        <div class="row bg-light py-4">
            ...
            $count_sql ="Select count(matricnumber) from student  WHERE student.group_id = '$stud_count'";
  ...
  <?php
    if(isset($_GET['grp_id'])){
      ...
      $count_sql ="Select * from student WHERE student.group_id = '$id'";
      $count=mysqli_query($conn, $count_sql);?>
       <div class="container">
       <div class="lead">List of Student in Group <?php echo $stud_count;?></div>
